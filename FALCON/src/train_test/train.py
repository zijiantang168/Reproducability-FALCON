...
def train(net,
    ...
    outputfile = open(str(start_time) + ".txt", "w")
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if iteration % 100 == 99:
                now_time = time.time()
                accuracy_xy = (float(100) * float(correct) / float(total))
                print('accuracy: %f %%; loss: %f; time: %ds'
                      % (accuracy_xy, loss, (now_time - last_time)))

                outputfile.write("%f %f %d\n" % (accuracy_xy, loss, iteration))
                outputfile.flush()
                if log != None:
